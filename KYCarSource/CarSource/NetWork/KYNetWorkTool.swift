...
class KYNetWorkTool: NSObject {
    ...
    var localPara:[String:Any] = ["devicetype":DEVICE_TYPE,"versioncode":VERSION,"user_id":KYUtilControl.GET_USER_DEFAULTS(Key: "userID")!,"appkey":APP_KEY,"ts":"\(KYUtilControl.changeNowDateToTimeStamp())","oauth_token":KYUtilControl.GET_USER_DEFAULTS(Key: "oauth_token")!,"signa":KYUtilControl.md5(Str: "\(KYUtilControl.md5(Str: "\(FRONT_SALT)\(KYUtilControl.changeNowDateToTimeStamp())"))\(BEHIND_SALT)".uppercased())]
    
    func handleResponse(JSON json:JSON) -> result{
        if KYUtilControl.isBlankString(String: json["res_msg"].string) != "" {
...
//        localPara["id"] = num
//        localPara["pwd"] = KYUtilControl.encodingBase64(String: pass)
//        KYNetwork.requestWith(Method: .post, URL: HOST + LOGIN_API, Parameter: localPara, Token: nil) { (res) in
//            do {
...
//        KYNetwork.requestWith(Method: .post, URL: HOST + MAIN_API, Parameter: localPara, Token: nil) { (res) in
//            do {
//                let json = try JSON(data:res.data!)
//                if JSON.null != json {
//                    let aResult = self.handleResponse(JSON: json)
...
//        KYNetwork.requestWith(Method: .post, URL: HOST + REGISTER_PIC_CODE_API, Parameter: localPara, Token: nil) { (res) in
...
//        }
    ...
    func getSmsCodeWith(Dic dic:Dictionary<String, String>, Handler comp:@escaping ((result) -> Void)){
        ...
        KYNetwork.requestWith(Method: .post, URL: urlStr, Parameter: localDic, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func registerWith(PhoneNum phoneNum:String, InvestCode investCode:String?, SmsCode smscode:String, UserType type:String, Password password:String, Handler comp:@escaping ((result) -> Void)){
        ...
        KYNetwork.requestWith(Method: .post, URL: HOST + REGISTER_API, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func checkMobilePhoneWhenForgetPassword(PhoneNum phoneNum:String, SmsCode smscode:String, Handler comp:@escaping ((result) -> Void)){
        ...
        localPara["code"] = smscode
        KYNetwork.requestWith(Method: .post, URL: HOST + FORGET_PASS_FIRST_STEP_API, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func changePasswordWith(PhoneNum phoneNum:String, Password password:String, Session_id session_id:String, Handler comp:@escaping((result) -> Void)){
        localPara["phone_or_email"] = phoneNum
        localPara["pwd"] = KYUtilControl.encodingBase64(String: password)
        localPara["session_id"] = session_id
        KYNetwork.requestWith(Method: .post, URL: HOST + CHANGE_PASSWROD_API, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func getInvestListWith(Category cate:String, Page page:String, Handle comp:@escaping((result) -> Void)){
        ...
        KYNetwork.requestWith(Method: .post, URL: HOST + INVEST_LIST_API, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func getCunguanUrlWith(UserType type:String, Handler comp:@escaping((result) -> Void)) {
        ...
        KYNetwork.requestWith(Method: .post, URL: HOST + CUNGUAN_API, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    //获取出借订单号
    func getInvestNoWith(InvestID investId:String, Money money:String, RateID rateId:String, UserID userId:String = KYUtilControl.GET_USER_DEFAULTS(Key: "userID") as! String, UuID uuid:String, SessionID session_id:String, IsRewardInvest isRewardInvest:String, RedBonusID bonusId:String, InterestID interestId:String, UseTaste useTaste:String, CheckPass checkPass:String, DoubleReturnType doubleReturnType:String, Handler comp:@escaping((result) -> Void)) {
        ...
        KYNetwork.requestWith(Method: .post, URL: HOST + GET_INVEST_NO, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func checkDirectInvestPass(Password pass:String, UUID uuid:String, InvestID investId:String, Handler comp:@escaping((result) -> Void)){
        ...
        localPara["id"] = investId
        localPara["pwd"] = pass
        KYNetwork.requestWith(Method: .post, URL: HOST + CHECK_PWD, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func getInvestDetailWith(UserID userId:String, UUID uuid:String, NID nid:String, Handler comp:@escaping((result) -> Void)) {
        localPara["id"] = nid
        localPara["uuid"] = uuid
        KYNetwork.requestWith(Method: .post, URL: HOST + INVEST_DETAIL, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func getInvestDetailAfterUpSwipeWith(UUID uuid:String, TypeID typeId:String, Handler comp:@escaping((result) -> Void)) {
        ...
        KYNetwork.requestWith(Method: .post, URL: HOST + INVEST_DETAIL_DEATIL, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func getInvestHistoryWith(UUID uuid:String, NID nid:String, Page page:String, Handler comp:@escaping((result) -> Void)) {
        ...
        localPara["id"] = nid
        KYNetwork.requestWith(Method: .post, URL: HOST + INVEST_HISTORY, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
    ...
    func getMineData(Handler comp:@escaping((result) -> Void)) {
        KYNetwork.requestWith(Method: .post, URL: HOST + MINE_CENTER_API, Parameter: localPara, Token: nil) { (res) in
            do {
                let json = try JSON(data:res.data!)
                if JSON.null != json {
                    let aResult = self.handleResponse(JSON: json)
